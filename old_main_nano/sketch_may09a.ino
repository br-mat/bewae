...
byte slave_adr = 9; //adress of slave esp for wifi
...
void loop() {
  ...
  data2[0] = bme280.getPressure();
  data2[1] = bme280.getTemperature();
  data2[2] = bme280.getHumidity();
  data2[3] = 0;
  data2[4] = 0;
  data2[5] = 0;
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //watering(s0_mux_1, s2_mux_1, s1_mux_1, 10000, 1, pump1, q4_npn_ensh, vent_pwm);
...
void controll_mux(uint8_t control_pins[], uint8_t channel, uint8_t sipsop, uint8_t enable, String mode, int *val){
  ...
  if(mode == String("read")){
    ...
    digitalWrite(enable, LOW);
    pinMode(sipsop, OUTPUT); //make sure its on input
    digitalWrite(sipsop, LOW);
    delay(1);
    pinMode(sipsop, INPUT); //make sure its on input
    delay(250);
