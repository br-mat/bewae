...
unsigned long phase_time = 0; //timemark variable
...
byte transmitt(int data[]){
  // data - int array should be packed in max [28]
  byte rstatus = 0;
  if(sizeof data < 28 * sizeof (int) +1){
    //int tsize = sizeof data;
    Serial.println("Debugstatement inside if positive");
    
    int tsize = 14 * sizeof (int);
    byte bdata[tsize+2]={0}; //+2 because 1 transmission byte and the last byte cannot be read correctly
...
void setup() {
  ...
  //TWBR = 78;  //78 --> 25 kHz
  //TWBR = 158;  //158 --> 12,5 kHz
  TWBR = 254;
  //TWSR |= bit (TWPS0);  // change prescaler did not work for me
  Serial.println("asd");
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // setup pin mode
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void loop() {
  ...
  if(rtc_status != 0){
    Serial.println("Error: rtc device not available!");
  }
  ...
  if(hour_ != hour1){
    up_time += 60UL*1000UL; //refresh the up_time every hour, no need for extra function or lib to calculate the up time
    if((hour1 == 7) | (hour1 == 21)){
      thirsty = true;
    }
  }
  unsigned long time1 = up_time+sec1*1000+min1*1000*60;
  ...
  if(time1-phase_time > 1200000UL)
  {
    ...
    // --- Datalog INFLUXDB ---
    TWBR=254; //make sure that IIC clock speed is low enough for esp01
    ...
    digitalWrite(q2_npn_sens, HIGH); //deactivate sensors
  }
